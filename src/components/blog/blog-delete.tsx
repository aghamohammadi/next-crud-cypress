...
import { remove } from "@/utils/services/blog-service";
...
const BlogDelete = ({ postData }) => {
    ...
    const { register, handleSubmit, formState: { errors }, reset } = useForm({
        defaultValues: {
            ...
            insertDate: postData?.insertDate,
    ...
    const handleSubmitAdd = async (formData) => {
        ...
        const result = await remove(formData.id);
        if (result.status === 200) {
            // revalidate({ tag: ['updated'], path: ['/admin', `/admin/edit/${postData.id}`] });
    ...
    return ( 
        <form onSubmit={handleSubmit(handleSubmitAdd)} method="post">
            ...
            <div className="relative z-0 w-full mb-6 group">
                <input type="text" id="title" disabled {...register("title")} className="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " />
            ...
            <div className="relative z-0 w-full mb-6 group">
                <textarea id="summary" rows={3} disabled {...register("summary")} className="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " />
            ...
            <div className="grid md:grid-cols-2 md:gap-6">
                <div className="relative z-0 w-full mb-6 group">
                    <input type="date" disabled {...register("insertDate",)} id="insertDate" className="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " />
                ...
                <div className="relative z-0 w-full mb-6 group">
                    <label className="relative inline-flex items-center mt-4 cursor-pointer">
                        <input disabled type="checkbox" {...register("published")} id="published" value="" className="sr-only peer" />
            ...
            <div className="relative z-0 w-full mb-6 group">
                <div className="relative z-0 w-full mb-6 group">
                    <a className="inline-block" href={postData.imagePath} target="_blank">
                        <img id="image-preview" className="w-40" src={postData.imagePath} />
                    </a>
                </div>                
            </div>
            <div className="relative z-0 w-full mb-6 group">
                <textarea rows={15} disabled {...register("content")} id="content" className="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " />
