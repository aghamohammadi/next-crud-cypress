...
import { add, edit } from "@/utils/services/blog-service";
...
const BlogEditForm = ({postData}) => {
    const [previewImage, setPreviewImage] = useState(postData.imagePath);
    const [imagePath, setImagePath] = useState(postData.imagePath);
    ...
    const handleSubmitAdd = async (formData) => {
        ...
        form.append('imageFile', formData.imageFile && formData.imageFile.length>0 ? formData.imageFile[0]:null)
        form.append('data', JSON.stringify(data))

        const result = await edit(form);
            ...
            <form onSubmit={handleSubmit(handleSubmitAdd)} method="post">
                ...
                <div className="grid md:grid-cols-2 md:gap-6">
                    ...
                    <div className="relative z-0 w-full mb-6 group">
                        <label className="relative inline-flex items-center mt-4 cursor-pointer">
                            <input type="checkbox" {...register("published")} value="" id="published" className="sr-only peer" />
                ...
                <div className="relative z-0 w-full mb-6 group">
                    <div className="relative z-0 w-full mb-6 group">
                        ...
                        {imagePath && <button onClick={onDeleteImage} className="align-top ms-2 bg-red-500 hover:bg-red-700 transition text-white font-bold py-2 px-4 rounded">
                            <svg className="h-6 w-6 " fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" aria-hidden="true"><path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12"></path></svg>
                        </button>  }
                    </div>
                    <div className="relative z-0 w-full mb-6 group">
                        <input type="file" {...register("imageFile", { required: imagePath?false: true, onChange: (e) => { onChangeImage(e) }, })} id="imageFile" className="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" />
